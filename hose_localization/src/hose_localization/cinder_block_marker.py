...
class ObjectStatus:
    ...
    def __init__(self):
        ...
        self.default_radius = 0.19
        self.default_height = 0.39
...
class CinderBlockMarker(MoveableButtonMarker):
    def __init__(self, marker_namespace, frame='/world', marker_tran = numpy.eye(4)):        
        self.status = ObjectStatus()
        marker_tran[2,3] = self.status.radius/2
        MoveableButtonMarker.__init__(self, marker_namespace, frame, marker_tran)

        self.left_pose_topic = "/leftFoot/set_absolute_pose"
        self.right_pose_topic = "/rightFoot/set_absolute_pose"
    ...
    def create_marker(self):
        ...
        self.int_marker.name += "_cinder_block"
    ...
    def make_object_marker(self):
        ...
        self.object_marker.scale.x = self.status.height
        self.object_marker.scale.y = self.status.radius
        self.object_marker.scale.z = self.status.radius 
        pose_offset = Pose()
    ...
    def send_pregrasp_transform(self):
        #send the pregrasp transform in the block's frame
        #target_pose = self.status.pose_stamped.pose
            
        
        foot_pose = PoseStamped()
        foot_pose.pose.position.z = self.status.radius/2

        
        foot_pose.header.frame_id = self.int_marker.name

        self.send_hand_pose(foot_pose)
...
if __name__ == '__main__':
    rospy.init_node('block_marker_test')

    block_marker = CinderBlockMarker('cinder_block_test','/leftFoot')
    block_marker.initialize_menu()
    loop = rospy.Rate(20)

    server = InteractiveMarkerServer(block_marker.marker_namespace)
    block_marker.create_marker()

    while not rospy.is_shutdown():        
        block_marker.update()
        server.clear()
        server.insert(block_marker.int_marker, block_marker.int_marker_feedback_cb)
        block_marker.menu_handler.apply(server, block_marker.int_marker.name)
        block_marker.update()
        server.applyChanges()
