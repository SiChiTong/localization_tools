...
class FootMarker (MoveableButtonMarker):
    def __init__(self, marker_namespace, 
        ...
        self.update_menu("snap to other foot",self.align_to_other_foot,[])
        self.update_menu("send_pose",self.publish_pose,[])
        
        self.align_to_foot()
        self.pose_publisher = rospy.Publisher(self.side + "_foot_pose", PoseStamped)
    ...
    def create_marker(self):
        ...
        self.int_marker.name=self.side + "_foot_marker"
    ...
    def align_to_foot(self):
        
        if self.tf_listener.canTransform(self.hand_base_frame,
                                         self.marker_pose_stamped.header.frame_id,
                                         rospy.Time(0)):            
            foot_tf = self.tf_listener.lookupTransform(self.marker_pose_stamped.header.frame_id,
                                                          self.hand_base_frame, rospy.Time(0))
            self.set_pose(PoseFromTransform(TransformFromComponents(*foot_tf)))

    def align_to_other_foot(self):
        other_foot = 'left'
        if self.marker_namespace.find(other_foot) >= 0:
            other_foot = 'right'
        other_foot_marker_frame = "/%s_foot/%sAR"%(other_foot, other_foot[0].upper())
            
        if self.tf_listener.canTransform(self.marker_pose_stamped.header.frame_id,
                                         other_foot_marker_frame,
                                         rospy.Time(0)):            
            foot_tf = self.tf_listener.lookupTransform(other_foot_marker_frame,
                                                          self.marker_pose_stamped.header.frame_id,
                                                           rospy.Time(0))
            foot_pose = PoseFromTransform(TransformFromComponents(*foot_tf))
            foot_pose.position.z += .2
            self.set_pose(foot_pose)
...
if __name__ == '__main__':
    ...
    namespace = rospy.get_param('~marker_namespace','left_foot_marker' )

    print namespace
    grasper_marker = FootMarker(namespace,"/leftFoot")
    #ipdb.set_trace()
